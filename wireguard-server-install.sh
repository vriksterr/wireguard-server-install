...
function checkOS() {
	...
	if [[ ${OS} == "debian" || ${OS} == "raspbian" ]]; then
		...
		if [[ ${VERSION_ID%%.*} -lt 3 ]]; then
			echo "Your version of Alpine Linux (${VERSION_ID}) is not supported. Please use Alpine Linux 3.0 or later"
			exit 1
		fi
		OS=alpine
	else
		echo "Looks like you aren't running this installer on a Debian, Ubuntu, Fedora, CentOS, AlmaLinux, Oracle, Arch Linux, or Alpine Linux system"
		exit 1
	fi
}
